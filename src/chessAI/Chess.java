...
//		int skewScore2 = getSkewScore2(x, y, 1);
	...
	private int getScoreBySituation(int count1, int leftStatus, int rightStatus) {
		...
		// 四子情况
		if (count1 == 4) {
			if (leftStatus == 1 && rightStatus == 1) //活四，分数+50000
				score += 50000;
			if ((leftStatus == 2 && rightStatus == 1) || (leftStatus == 1 && rightStatus == 2))  //冲四，分数+3000
				score += 3000;
			if (leftStatus == 2 && rightStatus == 2) //死四，分数+1000
				score += 1000;
		}

		//三子情况
		if (count1 == 3) { 
			if (leftStatus == 1 && rightStatus == 1)  //活三+3000
				score += 3000;
			if ((leftStatus == 2 && rightStatus == 1) || (leftStatus == 1 && rightStatus == 2)) //冲三+1000
				score += 1000;
			if (leftStatus == 2 && rightStatus == 2) //死三+500
				score += 500;
		}
		
		//二子情况
		if (count1 == 2) {
			if (leftStatus == 1 && rightStatus == 1) //活二+500
				score += 500;
			if ((leftStatus == 2 && rightStatus == 1) || (leftStatus == 1 && rightStatus == 2)) //冲二+200
				score += 200;
			if (leftStatus == 2 && rightStatus == 2) //死二+100
				score += 100;
		}
		
		//一子情况
		if (count1 == 1) {
			if (leftStatus == 1 && rightStatus == 1) //活一+100
				score += 100;
			if ((leftStatus == 2 && rightStatus == 1) || (leftStatus == 1 && rightStatus == 2)) //冲一+50
				score += 50;
			if (leftStatus == 2 && rightStatus == 2) //死一+30
				score += 30;
		}
